...
**`scripts/check.sh` — is the pack internally consistent?** 36 numbered sections plus 3b, 3c and 13a — **<!-- count:checks -->39<!-- /count --> checks**; the header comment is their single enumeration, updated in the
...
**`docs/guard-matrix.sh` — does each guard fire?** <!-- count:matrix-cases -->156<!-- /count --> cases, each
