...
**`scripts/check.sh` — is the pack internally consistent?** 37 numbered sections plus 3b, 3c and 13a — **<!-- count:checks -->40<!-- /count --> checks**; the header comment is their single enumeration, updated in the
...
**`docs/guard-matrix.sh` — does each guard fire?** <!-- count:matrix-cases -->158<!-- /count --> cases, each
