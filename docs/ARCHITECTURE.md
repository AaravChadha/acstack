...
**`scripts/check.sh` — is the pack internally consistent?** 45 numbered sections plus 3b, 3c and 13a — **<!-- count:checks -->48<!-- /count --> checks**; the header comment is their single enumeration, updated in the
...
**`docs/guard-matrix.sh` — does each guard fire?** <!-- count:matrix-cases -->201<!-- /count --> cases, each
