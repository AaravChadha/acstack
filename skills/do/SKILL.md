...
Deltas: `references/tickets-mode.md` — read it only when the resolved
...
config sets `mode: hackathon`, and read it **before** "Before starting"
below, because in hackathon mode PLAN.md is read from the task's worktree,
not the main checkout. Before the task, the session gets onto its own task
branch built from the local `main` (creating the worktree if it starts in
the main checkout, and stopping on unlanded or uncommitted work);
